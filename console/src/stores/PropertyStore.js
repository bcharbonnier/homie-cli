...
function handleNodePropertyDefinition(state, { deviceId, nodeId, property }) {
  ...
  return state.set(
    deviceId,
    device.withMutations(map => {
      const nodes = map.get("nodes", Immutable.Map());
      return map.set(
        "nodes",
        nodes.withMutations(nodeMap => {
...
class PropertyStore extends MapStore {
  initialize() {
    this.addAction(
      ActionTypes.DEVICE_NODE_PROPERTY_DEFINITION,
      handleNodePropertyDefinition
    );
    this.addAction(ActionTypes.DEVICE_NODE_TYPE_UPDATE, handleNodeTypeUpdate);
    this.addAction(
      ActionTypes.DEVICE_NODE_PROPERTY_VALUE_UPDATE,
      handleNodePropertyValueUpdate
    );
